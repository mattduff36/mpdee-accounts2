...
export default async function SettingsPage() {
  ...
  async function updateSettings(formData: FormData) {
    ...
    const data = {
      ...
      emailSubjectTemplate: String(formData.get("emailSubjectTemplate") || ""),
      emailBodyTemplate: String(formData.get("emailBodyTemplate") || ""),
    }
      ...
      <form action={updateSettings} className="space-y-6">
        <div className="rounded-lg border bg-white p-6 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input name="tradingName" defaultValue={settings?.tradingName || ""} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input name="county" defaultValue={settings?.county || ""} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input name="companyNumber" defaultValue={settings?.companyNumber || ""} />
            </div>
            <div>
        ...
        <div className="rounded-lg border bg-white p-6 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <Input name="invoicePrefix" defaultValue={settings?.invoicePrefix || "INV"} />
            </div>
            <div>
              ...
              <Input
                ...
                type="number"
                defaultValue={settings?.defaultPaymentTerms || 30}
              />
            </div>
          </div>
        </div>
        <div className="rounded-lg border bg-white p-6 space-y-4">
          ...
          <div>
            ...
            <Input name="defaultVatRate" type="number" defaultValue={settings?.defaultVatRate || 20} />
          </div>
        </div>
        <div className="rounded-lg border bg-white p-6 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input name="emailFromAddress" type="email" defaultValue={settings?.emailFromAddress || ""} />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Invoice email subject</label>
            <Input
              name="emailSubjectTemplate"
              defaultValue={
                settings?.emailSubjectTemplate || "Invoice {{invoiceNumber}} from {{businessName}}"
              }
            />
            <p className="mt-1 text-xs text-gray-500">
              Placeholders: {"{{invoiceNumber}}"}, {"{{businessName}}"}, {"{{clientName}}"}, {"{{amount}}"},{" "}
              {"{{dueDate}}"}, {"{{issueDate}}"}
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Invoice email body</label>
            <textarea
              name="emailBodyTemplate"
              rows={8}
              className="flex min-h-[160px] w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm"
              defaultValue={
...
If you have any questions about this invoice, please don't hesitate to contact us.`
            ...
            />
            <p className="mt-1 text-xs text-gray-500">
              Plain text with the same placeholders. Blank lines become paragraphs. Layout/chrome stay
